            ...
            <div className="flex w-full min-h-screen relative z-10">
                ...
                <motion.div initial={{ width: 256 }} animate={{ width: isSidebarOpen ? 256 : 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="border-r border-zinc-200 flex flex-col fixed h-full bg-white z-40 overflow-hidden shadow-xl">
                    ...
                    <nav className="flex-1 p-4 space-y-2 min-w-[256px]">
                        ...
                        <button onClick={() => setActiveView('competition')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all ${activeView === 'competition' ? 'bg-primary text-black font-bold' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900'}`}><Trophy size={20} /> Registration</button>
                        <button onClick={() => setActiveView('ads')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all ${activeView === 'ads' ? 'bg-primary text-black font-bold' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900'}`}><Megaphone size={20} /> Events & Ads</button>
